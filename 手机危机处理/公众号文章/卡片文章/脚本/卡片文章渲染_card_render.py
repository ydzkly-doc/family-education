...
# ---------- 本系列配色（《手机危机处理》· 暖陶土）----------
# ⭐ 全部取自本系列长文正文既有色值，系列内不新增色（SOP vision.md 硬规则）
#    长文 14 色：#b06a48 #93553A #3A322C #6B5D52 #5A6663 #8A5A2B
#                 #FAF3EC #FEF6EC #F6ECE2 #F0DDC4 #FDFAF5 #F5F3EF #FFFFFF #B4730F
MAIN  = '#b06a48'      # 主色 · 暖陶土（长文主色）
DEEP  = '#93553A'      # 深色 · 主标题（长文次色）
BAND  = '#F0DDC4'      # 细线 / 时间轴（长文边框色）
BG    = '#FAF3EC'      # 内容卡浅底（长文卡片底）
INK   = '#3A322C'      # 正文墨色（长文正文色）
BROWN = '#6B5D52'      # 正文（次墨色）
GREY  = '#5A6663'      # 次要文字（长文灰绿）
LIGHT = '#F6ECE2'      # 浅格（长文色）
PAPER = '#FEF6EC'      # 次浅格（长文色）
DIM   = '#8A5A2B'      # 弱化文字（长文色）
CREAM = '#FDFAF5'      # 封面/收尾卡浅色文字
GOLD  = '#F0DDC4'      # ⭐ 高亮块【底色】——必须浅：块内文字用 INK(#3A322C) 深色
...
#   原写法在模块导入时就 `os.makedirs(OUT)`，而 OUT 是"第1篇"的硬编码默认值，
#   导致①任何 import（含密度预估脚本）都会在 卡片文章/ 下凭空建出空目录
#   `卡片文章/发布包_第1篇_xxx/卡片发布包/`；②跑第2篇时若配置未生效也会错写进第1篇。
#   → 目录创建统一交给 main() 里"配置读取完成之后"执行（见文件末尾 `os.makedirs(OUT, exist_ok=True)`）。
...
NOTES = ['课程学习心得 · 家庭场景改编', '孩子又犯错以后 · 第1篇 / 共6篇']  # [0]=改编标注(P0) [1]=系列行
...
def draw_center_lines(draw, lines):
    ...
    for L in lines:
        if L['box']:
            ...
            draw.text((L['x'], L['y']), L['t'], font=L['f'], fill=L.get('tc', INK))
            BOXES.append(L['box'])
        else:
...
# ⭐ 逐张标注：改编类标注用暖沙色，纯身份/来源标注用中性色
# ⚠️ 对比度纪律（2026-09-17）：标注是 **P0 内容**，必须看得清——
#    浅色底上不得低于 2.0:1；实测初版 1.99:1 偏低，故整体加深。
def _note_color(text, light=False):
    """⭐ 标注颜色按"类型"分：
       改编/虚构 → 暖金（提醒有改编成分）
       真实记录/来源 → 中性色（本系列《手机危机处理》全部为**身份标注**，走此分支）
    对比度纪律（2026-09-17）：标注是 P0 内容，浅底上不得低于 2.0:1。"""
    if '改编' in text or '虚构' in text:
        return '#8a6b3a' if not light else '#e6d2a8'
    return '#6B5D52' if not light else '#d8d2c2'   # 身份标注：本系列次墨色（长文色）
...
def top_labels(draw, light=False):
    ...
    y = 34
    tw = draw.textlength(t, font=f)
    draw.text(((W - tw) / 2, y), t, font=f,
              fill='#d8cdb0' if light else '#6f7161')
...
def cover_card():
    ...
    items = [{'t': l['t'], 'f': font(fsize, True),
              'c': INK if l.get('hl') else CREAM,
              'hl': GOLD if l.get('hl') else None} for l in lns]
    ...
    if len(NOTES) > 1:
        ...
        if ny >= SAFE_16_9[0]:
            d.text(((W - tw1) / 2, ny), t1, font=f_note, fill='#ded3b4')
            BOXES.append(((W - tw1) / 2, ny, (W + tw1) / 2, ny + f_note.size))
        ny += f_note.size + 8
    note = _cover_note
    tw = d.textlength(note, font=f_note)
    _nc = '#eadfc0' if '改编' in note else '#ded8c8'
...
def soften_bg(im, blur=2.6, veil=0.34, veil_color='#1b1c17', top=0.46, bottom=0.42):
    """底图重处理：强柔化 + 整体压暗 + 上下渐暗
...
def closing_card():
    ...
    f_j = font(qsize, True)
    items = [{'t': q['t'], 'f': f_j,
              'c': INK if q.get('hl') else CREAM,
              'hl': GOLD if q.get('hl') else None} for q in quote]
...
def save(im, name, limit_kb=98):
    """写入带重试：Windows 下文件可能被预览/杀毒短暂占用

    ⭐ 2026-09-19：limit 由 100 → **98**（留 2KB 余量）。
    原实现取"第一个 ≤100KB 的 q 值"，必然**贴着上限**——实测收尾卡 102392 字节，
    余量仅 **8 字节**，与 C11「贴着上限写作＝把风险留给自己」同类。
    留 2KB 后画质差异肉眼不可见，但不再踩线。
    """
    import time
