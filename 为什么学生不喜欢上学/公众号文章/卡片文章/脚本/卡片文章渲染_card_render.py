...
# ---------- 本系列配色（《为什么学生不喜欢上学》· 青绿）----------
# 与长文正文/封面同色系，系列内不新增色值（SOP vision.md 硬规则）
MAIN  = '#3f6f6a'      # 主色 · 青绿（长文主色）
DEEP  = '#2f5753'      # 深色 · 主标题（长文深色）
BAND  = '#c4d9d5'      # 细线 / 时间轴
BG    = '#f4f6f5'      # 内容卡浅底（长文页面底色）
INK   = '#33302b'      # 正文墨色（长文正文）
BROWN = '#3a4a47'      # 正文（略偏墨绿）
GREY  = '#8f9c99'
LIGHT = '#eef4f3'      # 浅格（长文卡片浅底）
PAPER = '#f0f5f4'      # 次浅格
DIM   = '#7e8d8a'
CREAM = '#f2f7f6'      # 封面/收尾卡浅色文字
GOLD  = '#d9963f'      # 辅暖色 · 高亮块底（长文辅色）
...
NOTES = ['读书笔记 · 书中观点整理', '为什么学生不喜欢上学 · 第1篇 / 共8篇']  # [0]=改编标注(P0) [1]=系列行
...
def _note_color(text, light=False):
    ...
    return '#5f6152' if not light else '#d8d2c2'
...
def timeline_card(topic, head, nodes):
    ...
    # ⭐ 2026-09-19：节点间距可由配置覆盖（tl_gap）——
    #    实测 5 节点卡在固定 52 时密度偏高、4 节点卡偏低，需要按卡微调。
    _GAP = CFG.get('tl_gap', 52)

    # 先量总高（用于垂直居中）
    _h = f_topic.size + 24 + int(f_head.size * 1.34) * len(head_lines) + 50
    for _t, _body in nodes:
        _h += f_t.size + 20 + int(f_b.size * 1.64) * len(wrap(d, _body, f_b, MW)) + _GAP
    total = _h - _GAP
    ...
    for t, body in nodes:
        ...
        y = yy + _GAP
    d.rounded_rectangle([MARGIN + 17, line_top, MARGIN + 24, y - _GAP - 8], radius=3, fill=BAND)
    sig(d)
    return im
