...
_rnnn = paths.find_rnnn_model()
RNNN_DIR = os.path.dirname(_rnnn) if _rnnn else ""
...
DEFAULT = {
    ...
    #           首尾淡入淡出：避免开头/结尾的"咔"声与突然截断
    #           淡出需要总时长，由调用方传入（实测：不传则只做淡入）
...
# 预设：直接改参数，不用背滤镜语法
# 注意：「关闭」必须是**显式关掉每一环**，不能写成空字典
#      （空字典 merge 后仍等于 DEFAULT，看起来"关了"其实全开——踩过）
PRESETS = {
    "关闭": {"highpass": {"enabled": False}, "arnndn": {"enabled": False},
             "afftdn": {"enabled": False}, "deesser": {"enabled": False},
             "eq": [], "compressor": {"enabled": False},
             "loudnorm": {"enabled": False}, "alimiter": {"enabled": False}},
...
# spec 里 audio 段的"控制键"，其余键都被当作参数覆盖
AUDIO_CONTROL_KEYS = ("enabled", "preset", "override")
...
def overrides_from_spec(acfg: dict) -> dict:
    """
    从 spec 的 audio 段提取参数覆盖。
    除控制键（enabled/preset/override）外，**其余键都视为参数覆盖**，
    这样 `--set audio.arnndn.mix=0.9` 与 `audio.override.arnndn.mix` 两种写法都生效。
    """
    ov = {k: v for k, v in acfg.items() if k not in AUDIO_CONTROL_KEYS}
    extra = acfg.get("override") or {}
...
def config_from_spec(acfg: dict) -> dict:
    """spec 的 audio 段 → 最终参数配置（预设打底 + 覆盖）"""
    cfg = resolve(acfg.get("preset", "标准"))
    ov = overrides_from_spec(acfg)
    return merge(cfg, ov) if ov else cfg


def describe_preset(name: str) -> str:
    """给预设一句人类可读的说明（供 --list-presets 用）"""
    over = PRESETS.get(name)
    if over is None:
        return f"（未定义，将按标准处理）"
    if name == "关闭":
        return "完全关闭音频处理，音频原样保留"
    if not over:
        return "即标准默认（见下方参数表）"
    keys = []
    for k, v in over.items():
        if isinstance(v, dict) and v.get("enabled") is not None:
            keys.append(f"{k}{'开' if v['enabled'] else '关'}")
        elif isinstance(v, dict) and "mix" in v:
            keys.append(f"{k}强度{v['mix']}")
        elif isinstance(v, dict) and "I" in v:
            keys.append(f"目标{v['I']}LUFS")
        else:
            keys.append(k)
    return "、".join(keys)
...
# loudnorm 各 measured_* 参数的合法区间（超出会被 ffmpeg 拒绝并直接报错）
MEASURED_RANGES = {
    "input_i": (-99.0, 0.0),
    "input_tp": (-99.0, 99.0),
    "input_lra": (0.0, 99.0),
...
def valid_measured(m: dict | None) -> bool:
    """
    校验 loudnorm 的分析结果是否可用。

    ⛔ 必须校验：实测在极端素材（纯正弦波 + 数字静音）上，
       loudnorm 会吐出 `input_tp = 5825` 这种垃圾值，
       直接喂回 loudnorm 会报 `Value 5825 out of range [-99 - 99]` 并**整条命令失败**。
       取不到可信值时应当放弃两遍法、退回单遍，而不是把垃圾传下去。
    """
    if not m:
        return False
    try:
        for k, (lo, hi) in MEASURED_RANGES.items():
            v = float(m[k])
            if not (lo <= v <= hi):
                return False
...
def build_loudnorm(cfg: dict, measured: dict | None = None) -> str:
    ...
    if measured and ln.get("two_pass", True):
        if not valid_measured(measured):
            print("    ⚠️ 分析结果不可信（超出 loudnorm 合法区间），"
                  "自动退回单遍模式（动态自适应）")
...
def build_post(cfg: dict, duration: float | None = None) -> str:
    """
    loudnorm 之后的三道：真峰值限制 → 首尾淡入淡出 → 采样格式归一化（顺序不可换）

    duration 用于计算淡出起点；不给则只做淡入（淡出无从算起）。
    """
    parts = []
    al = cfg.get("alimiter", {})
    if al.get("enabled"):
    ...
    fd = cfg.get("fade", {})
    if fd.get("enabled"):
        fin = float(fd.get("in_s", 0.15) or 0)
        fout = float(fd.get("out_s", 0.25) or 0)
        if fin > 0:
            parts.append(f"afade=t=in:st=0:d={fin}")
        if fout > 0 and duration and duration > fout + 1:
            parts.append(f"afade=t=out:st={duration - fout:.3f}:d={fout}")

    parts.append(OUT_FORMAT)
    return ",".join(parts)


def build_full_chain(cfg: dict, measured: dict | None = None,
                     duration: float | None = None) -> tuple[str, str | None]:
    """组装完整链，返回 (滤镜串, 需要的 cwd)"""
    pre, cwd = build_chain(cfg)
    ln = build_loudnorm(cfg, measured)
    post = build_post(cfg, duration)
...
def analyze(src: str, pre_chain: str | None = None, cwd: str | None = None) -> dict | None:
    ...
    if not valid_measured(d):
        print()
        print("    ⚠️ 分析值超出合理区间（该素材可能含极长静音或极端电平），"
              "跳过达标预判；处理时将退回单遍 loudnorm。")
        return d
    try:
...
def process(src: str, dst: str, cfg: dict, workdir: str | None = None) -> bool:
    ...
    ln = build_loudnorm(cfg, measured)
    post = build_post(cfg, probe_duration(src))
