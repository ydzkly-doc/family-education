...
> 以下"三类必排问题（A/C/B）"对 section 版依然适用（data-id、div、颜色挂容器），继续遵守；**本文件自第 3 行起已全部改为 section 版规范，文中不再保留 table 骨架描述。**
...
| **B · 颜色挂 section** | `<section/th style="…color:…">` 且内部是裸文字 | 微信对 section 的 color 向下继承不稳定，文字丢色 | color 下沉到包住文字的 `<p>/<span>/<b>/<strong>` 自身；section 只留背景/内边距/宽度 |

一篇可同时含多类，**顺序固定 A → C → B**（先剥 data-id，再 div/h 转 p、白字拆 br，最后颜色下沉）。脚本幂等，可重复跑。

## 二、结构铁律

- 骨架：最外层满宽 `<section style="box-sizing:border-box;width:100%;background:页面底色;padding:16px 12px">` → 内层白色卡片 `<section style="box-sizing:border-box;width:100%;background:#fff;…">`。**卡片/色块/金句块/标签全部用 `<section>` + 内联 style（background/padding/border-radius/border-left）绘制**。
- **每个块 style 必含 `box-sizing:border-box;width:100%`**；**禁止 `<table>/<tr>/<td>` 布局、禁止容器级固定宽 `width/max-width:≥200px`、禁止 `align="center"` 居中嵌套**；留白靠 padding，不靠固定宽。
- **正文段落/小标题一律左对齐**；只有顶部标题块、金句块、陪伴卡、诗句、卡片标题在**该块自身 style** 写 `text-align:center;`。

## 三、标签白名单 / 黑名单

- **允许**：`section / p / span / b / strong / br`。
- **禁止**：`<table>/<tr>/<td>`（布局）、`<img>`（外链位图）、`class=`、`<style>` 块、`<div>`、`<ul>/<ol>/<li>`、`<h1>~<h4>`、任何 `data-*` 属性。
- 列表用 `<br>` + `·`/`①②③` 或 section 单元格实现。**全篇不使用 `<div>`**（任何位置都不写 div，文字一律 p/span/b/strong）。

## 四、颜色铁律（最关键）

- **文字颜色永远写在包住文字的 `<p>/<span>/<b>/<strong>` 的内联 style 上**。
- `<section>` 只承担 `background / padding / width / border / border-radius`，**不写 color**。
- 装饰竖条/空 section：只放 `background`+`width`、无文字、不写 color。
...
标题色块（坐标小字 + 主标题 + 「第X篇·共N篇|主题」）→ 开场叙事 → 台词卡（暖色）→ 竖条小标题×N → 正文段 → 方法/对照/流程 section 卡 → 「我后来试的几件小事」卡 → 金句居中块 → 预告/陪伴卡（灰底、无点赞转发指令）→ 来源声明（米白 `#faf9f6`+灰字、左对齐）。
...
**交付标准：所有正文 `合格 N/N`，且 data-* = 0、section 带 color = 0、白字块 br = 0、无 div/h1-4/ul/img/class/style块、**0 `<table>/<tr>/<td>`**、section/p/span/b 标签配平。**
