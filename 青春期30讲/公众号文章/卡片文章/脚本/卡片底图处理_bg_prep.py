...
def prep(src, dst, out_name, veil_a, veil_c, top_dark=0.0, blur=1.1):
    im = Image.open(src).convert('RGB')
    w, h = im.size
    print('\n▌ %s' % os.path.basename(src)[:64])
    print('  原图 %dx%d 比例 %.4f' % (w, h, w / h))

    # ① 去水印：顶锚裁到 3:4 高；若水印未出框则继续下裁
    ...
    cut_h = min(need_h, h)
    if wy0 and wy1 and wy1 >= cut_h:
        cut_h = wy0 - 8
        print('  ⚠️ 3:4 高度仍含水印 → 下裁至 y=%d' % cut_h)
